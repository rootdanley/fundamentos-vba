...
' Defina a faixa de c?lulas que cont?m a lista de crit?rios
    ...
    Dim pdfIndex As Long
    Dim pdfPaths() As String ' Array para armazenar os caminhos dos PDFs
    ...
    For Each criterio In criterios
        ...
        pdfFilePath = "C:\Users\rootd\OneDrive\Imagens\Saved Pictures\RelatorioDeAtividades-" & criterio.Row & Data & ".pdf"
        ReDim Preserve pdfPaths(pdfIndex)
        pdfPaths(pdfIndex) = pdfFilePath
        pdfIndex = pdfIndex + 1
    ...
    ' Loop através das linhas da coluna Email na planilha "settings"
        ...
        '     ' Preencher o email com informações e anexar os PDFs gerados
        '     With objEmail
        '         .To = strEmail
        '         .Subject = strMailSubject
        '         .Body = strMailBody
                
        '         ' Anexe todos os PDFs ao e-mail
        '         For i = LBound(pdfPaths) To UBound(pdfPaths)
        '             If pdfPaths(i) <> "" Then
        '                 .Attachments.Add pdfPaths(i)
        '             End If
        '         Next i
